...
def run():
	...
	for i in tqdm(range(100)):
        	...
        	Audios.objects.filter(id=pk).update(text=text, is_correct=True, user_name=5)
        	#user_name=2 is Whisper
        	#user_name=5 is Whisper3000 
        	sum_n = Users.objects.values('number').filter(id=5).first()
        	sum_n = int(sum_n['number']) + 1
        	Users.objects.filter(id=5).update(number=sum_n)
