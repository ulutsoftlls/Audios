...
class WhisperModel:
    def __init__(self, model_path="/mnt/ks/Works/asr/from_whisper/whisper-small-ky/checkpoint-2000", processor_path="/mnt/ks/Works/asr/from_whisper/whisper-small-ky"):
        self.model = WhisperForConditionalGeneration.from_pretrained(model_path).to("cuda:1")
    ...
    def generate_text_from_audio(self, audio_file_path):
        ...
        for key in inputs:
            inputs[key] = inputs[key].to("cuda:1")
        with torch.no_grad():
            outputs = self.model.generate(**inputs)

        generated_text = self.processor.decode(outputs[0].to("cuda:1"), skip_special_tokens=True)
