...
def run():
        data = Audios.objects.values_list('audio_file', 'text').filter(user_name=2, status=False, is_correct=True)
        #user_name=2 is Whisper
        ...
        base_file_name = 'exported_whisper_'  # Set your desired base file name
        timestamp = datetime.now().strftime('%d%H%M')  # Add a timestamp
